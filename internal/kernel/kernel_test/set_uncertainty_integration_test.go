...
func TestSetUncertainty_Export_Verify_Strict_Memory(t *testing.T) {
	...
	outU, err := cu.CreateUnit(ports.CreateUnitRequest{Key: "k1", Title: "t1", Description: "d", ActorID: "test"})
	if err != nil {
		t.Fatalf("create unit: %v", err)
	}
	cv := usecases.CreateVersion{Repo: repo, Audit: audit, Clock: clock}
	outV, err := cv.CreateVersion(ports.CreateVersionRequest{UnitKey: "k1", Label: "lbl", Content: "c", ActorID: "test"})
	if err != nil {
	...
	_, err = su.SetUncertainty(ports.SetUncertaintyRequest{UnitKey: "k1", VersionID: outV.VersionID, BodyBytes: uJSON, ActorID: "test"})
	if err != nil {
	...
	res, err := ver.VerifyAudit(ports.VerifyAuditRequest{UnitKey: "k1", StrictHash: true})
	if err != nil {
